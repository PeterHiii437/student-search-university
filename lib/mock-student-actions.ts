...
async function loadStudents(): Promise<Student[]> {
  ...
  studentCache = lines.slice(1).map((line, idx) => {
    const cols = line.split(';')
    const record: Record<string, string> = {}
    headers.forEach((h, i) => (record[h] = cols[i] || ''))
    const total = record['Diem_TT'] ? Number(record['Diem_TT']) : null
    const student: Student = {
      id: Number(record['STT_InGiayBao'] || idx + 1),
      mssv: record['MaSV'],
      ho_ten: record['HoTen'],
...
export async function searchStudent(_prevState: unknown, formData: FormData): Promise<{ success: boolean; student?: Student; error?: string }> {
  if (!formData) {
  ...
  const student = students.find(
    s => s.mssv === searchTerm || s.so_bao_danh === searchTerm || s.cccd === searchTerm
  )
...
export async function searchStudentById(query: string): Promise<{ success: boolean; student?: Student; error?: string }> {
  if (!query || query.trim() === '') {
    return { success: false, error: 'Vui lòng nhập MSSV, Số báo danh hoặc CCCD' }
  }
...
export async function getStudentStatistics() {
  ...
  const approvedCount = students.filter(s => s.trang_thai_duyet === 'approved').length
  const pendingCount = students.filter(s => s.trang_thai_duyet === 'pending').length
  const rejectedCount = students.filter(s => s.trang_thai_duyet === 'rejected').length
